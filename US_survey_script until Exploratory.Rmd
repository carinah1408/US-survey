---
title: "script_US_survey (without Exploratory H7 and MLM)"
author: "CH"
date: "28/11/2020"
...
# transform factor variables in new dataset
...
# boxplots (visual outlier inspection)
...
# histogram (visual frequency inspection)
...
ggplot(US_data, aes(x=secident, fill=vote))+
  geom_histogram(aes(y=..density..),binwidth=1, alpha=.5, position="identity") +
  labs(x= "Secure ingroup identification", y= "Frequency") +
  scale_fill_manual(name = "Vote", 
      labels = c("Democratic Party", "Republican Party", NA), 
      values = c("#58a6a6", "#efa355", NA)) +
  theme_light()
...
# no indication of acquiescent responding
```
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean approval for harsh measurements against immigrants and refugees") +
  coord_flip() +
  theme_classic()
fc_plot
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean estimated agreement for harsh measurements against immigrants and refugees (%)") +
  coord_flip() +
  theme_classic()
fc_plot
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean approval for perception that Whites are unfairly affected by affirmative action") +
  coord_flip() +
  theme_classic()
fc_plot
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean estimated agreement for unfair affection by affirmative action (%)") +
  coord_flip() +
  theme_classic()
fc_plot
...
# descriptive statistics for plots (plot itself was generated in Excel)
pi_plot <- US_t1 %>%
  dplyr::group_by(., vote) %>%
...
chisq.test(unexp_Rep$time, unexp_Rep$sign)
```
